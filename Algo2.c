 ...
 * sont mis en commentaire en général juste au-dessus du CHECK si jamais
 * celui-ci échoue.
 */
...
int NTAZ_RTSF(Liste L) { return NTAZ_SF(L, 0); }
...
Liste TuePosRecAux(Liste L, int compteur) {
    ...
    L->suite = TuePosRecAux(L->suite,
                            compteur + 1);  // On traite la suite de la liste
                                            // d'abord puis la valeur actuelle

    if (L->valeur ==
        compteur) {  // Si la valeur est egale a la position, on supprime
        Liste suite = L->suite;
        free(L);
        return suite;
    } else {
...
void TuePosRec(Liste* L) { *L = TuePosRecAux(*L, 1); }
...
void TuePosIt(Liste* L) {
    ...
    while (actuel != NULL) {  // On s'arrete quand on a plus rien a traiter
        if (actuel->valeur ==
            ...
            if (precedent == NULL) {  // Si on est au debut de la liste, on
                                      // decale la tete de liste
                *L = actuel->suite;
                free(actuel);
                actuel = *L;
        ...
        } else {  // Si la valeur n'est pas egale a la position, on passe
                  // simplement a l'element d'apres
            precedent = actuel;
            actuel = actuel->suite;
        }
        compteur++;
...
Liste TueRetroPosAUX(Liste L, int* compteur_retro) {
    if (L == NULL) {
        *compteur_retro = 0;  // On compte les positions depuis la fin en
                              // remontant la recursivite
        return NULL;
    }
    ...
    if (true) {
        {
            Liste L1 = NULL;
            Liste L2 = NULL;
            // printf("PlusCourteRec(L1, L2) = %d, PlusCourteIter(L1, L2) =
            // %d\n",
